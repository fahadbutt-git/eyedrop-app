...
// ─── Seed formulary data (matches original Excel Setup sheet) ───────────────
...
function FormField({ label, value, onChange, placeholder, width="100%" }) {
  return (
    <div style={{ width, marginBottom:10 }}>
      ...
      <input value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
        style={{ width:"100%", padding:"8px 12px", borderRadius:7, border:"1.5px solid #cbd5e0", fontSize:13, color:"#2d3748", outline:"none", background:"#fff", boxSizing:"border-box" }}
      />
    </div>
  );
}
...
export default function App() {
  ...
  // ── Shared formulary state ──
  const [formulary, setFormulary]   = useState(SEED_FORMULARY);
  const [editingId, setEditingId]   = useState(null);   // id of row being edited inline
  const [editBuf,   setEditBuf]     = useState({});     // draft values while editing
  const [addingNew, setAddingNew]   = useState(false);  // new-row form open?
  ...
  const displayedFormulary = [...formulary]
    .filter(m =>
      m.brand.toLowerCase().includes(searchQ.toLowerCase()) ||
      m.generic.toLowerCase().includes(searchQ.toLowerCase()) ||
      m.lu.toLowerCase().includes(searchQ.toLowerCase())
    )
    .sort((a, b) => {
  ...
  const updateDrop = (side, index, field, value) => {
    ...
    setter(prev => {
      ...
      if (field === "brand" && value) {
        ...
        if (med) {
          next[index].freq  = med.defaultFreq;
          next[index].drops = med.defaultDrops;
        }
      }
      return next;
    });
  ...
  const allDrops = [
    ...activeRight.map(d => ({ ...d, eye: "Right Eye (OD)" })),
    ...activeLeft.map(d =>  ({ ...d, eye: "Left Eye (OS)"  })),
  ];
  const mergedDrops = [];
  allDrops.forEach(d => {
  ...
  const EditCell = ({ field, type="text" }) => {
    if (type === "freq") return (
      <select value={editBuf[field]} onChange={e => setEditBuf(b => ({ ...b, [field]: Number(e.target.value) }))}
        style={{ width:"100%", padding:"5px 8px", borderRadius:6, border:"1.5px solid #4299e1", fontSize:12, outline:"none" }}>
    ...
    if (type === "drops") return (
      <select value={editBuf[field]} onChange={e => setEditBuf(b => ({ ...b, [field]: Number(e.target.value) }))}
        style={{ width:"100%", padding:"5px 8px", borderRadius:6, border:"1.5px solid #4299e1", fontSize:12, outline:"none" }}>
        {DROP_OPTIONS.map(d => <option key={d} value={d}>{d}</option>)}
      </select>
    );
    return (
      <input value={editBuf[field] || ""} onChange={e => setEditBuf(b => ({ ...b, [field]: e.target.value }))}
        style={{ width:"100%", padding:"5px 8px", borderRadius:6, border:"1.5px solid #4299e1", fontSize:12, outline:"none", boxSizing:"border-box" }}
      />
    );
  };
  ...
  // ══════════════════════════════════════════════════════════════════════════
  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#0f2044 0%,#1a3a6b 50%,#0f2044 100%)", fontFamily:"'Segoe UI',system-ui,sans-serif", color:"#1a202c" }}>

      {/* ── HEADER ── */}
      <div style={{ background:"rgba(255,255,255,0.05)", borderBottom:"1px solid rgba(255,255,255,0.1)", padding:"14px 28px", display:"flex", alignItems:"center", justifyContent:"space-between", backdropFilter:"blur(10px)" }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          <div style={{ width:40, height:40, borderRadius:"50%", background:"linear-gradient(135deg,#63b3ed,#4299e1)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, boxShadow:"0 4px 15px rgba(66,153,225,0.4)" }}>👁</div>
          <div>
            <div style={{ fontSize:19, fontWeight:700, color:"#fff", letterSpacing:"-0.3px" }}>Eye Drop Schedule</div>
            <div style={{ fontSize:11, color:"#90cdf4", marginTop:1 }}>Patient Medication Tracker</div>
          </div>
        </div>
        <div style={{ display:"flex", gap:6 }}>
          {tabs.map(t => (
            <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ padding:"8px 16px", borderRadius:8, border:"none", cursor:"pointer", fontSize:12, fontWeight:600, transition:"all 0.2s", background: activeTab===t.id ? "linear-gradient(135deg,#4299e1,#3182ce)" : "rgba(255,255,255,0.1)", color: activeTab===t.id ? "#fff" : "#90cdf4", boxShadow: activeTab===t.id ? "0 4px 15px rgba(66,153,225,0.35)" : "none" }}>
              {t.label}
            </button>
          ))}
        </div>
      ...
      <div style={{ padding:"24px 28px" }}>
          ...
          <div>
            ...
            <div style={{ background:"rgba(255,255,255,0.95)", borderRadius:14, padding:"16px 22px", marginBottom:18, boxShadow:"0 8px 32px rgba(0,0,0,0.25)", display:"flex", alignItems:"center", gap:14, flexWrap:"wrap" }}>
              <div>
                ...
                <div style={{ fontSize:12, color:"#718096", marginTop:1 }}>{formulary.length} medications · click any row to edit</div>
              </div>
              <input value={searchQ} onChange={e => setSearchQ(e.target.value)} placeholder="🔍  Search brand, generic, or LU code…"
                style={{ flex:1, minWidth:200, padding:"9px 14px", borderRadius:8, border:"1.5px solid #cbd5e0", fontSize:13, outline:"none" }}
              />
              <button onClick={() => { setAddingNew(true); setEditingId(null); }} disabled={addingNew}
                style={{ background: addingNew ? "#e2e8f0" : "linear-gradient(135deg,#48bb78,#38a169)", color: addingNew ? "#a0aec0" : "#fff", border:"none", borderRadius:8, padding:"9px 20px", fontSize:13, fontWeight:700, cursor: addingNew ? "default" : "pointer", boxShadow: addingNew ? "none" : "0 4px 12px rgba(72,187,120,0.35)" }}>
                + Add Medication
              </button>
            </div>
            ...
            <div style={{ background:"rgba(255,255,255,0.97)", borderRadius:14, overflow:"hidden", boxShadow:"0 8px 32px rgba(0,0,0,0.25)" }}>
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", minWidth:780 }}>
                  <thead>
                    <tr style={{ background:"linear-gradient(135deg,#1a365d,#2a4a8a)" }}>
                      ...
                      <SortTh col="generic"      style={{ ...thBase, textAlign:"left",   width:"35%"  }}>Generic Name</SortTh>
                  ...
                  <tbody>
                    ...
                    {addingNew && (
                      <tr style={{ background:"#fffff0", borderBottom:"2px solid #f6e05e" }}>
                        <td style={{ ...tdBase, paddingLeft:14, paddingRight:8 }}>
                          <input value={newMed.brand} onChange={e => setNewMed(m => ({ ...m, brand:e.target.value }))} placeholder="Brand name"
                            style={{ width:"100%", padding:"5px 8px", borderRadius:6, border:"1.5px solid #f6ad55", fontSize:12, outline:"none", boxSizing:"border-box" }} />
                        </td>
                        <td style={{ ...tdBase }}>
                          <input value={newMed.generic} onChange={e => setNewMed(m => ({ ...m, generic:e.target.value }))} placeholder="Generic name / concentration"
                            style={{ width:"100%", padding:"5px 8px", borderRadius:6, border:"1.5px solid #f6ad55", fontSize:12, outline:"none", boxSizing:"border-box" }} />
                        </td>
                        <td style={{ ...tdBase }}>
                          <select value={newMed.defaultFreq} onChange={e => setNewMed(m => ({ ...m, defaultFreq:Number(e.target.value) }))}
                            style={{ width:"100%", padding:"5px 8px", borderRadius:6, border:"1.5px solid #f6ad55", fontSize:12, outline:"none" }}>
                            {FREQ_OPTIONS.map(f => <option key={f} value={f}>{f}x/day</option>)}
                          </select>
                        </td>
                        <td style={{ ...tdBase }}>
                          <select value={newMed.defaultDrops} onChange={e => setNewMed(m => ({ ...m, defaultDrops:Number(e.target.value) }))}
                            style={{ width:"100%", padding:"5px 8px", borderRadius:6, border:"1.5px solid #f6ad55", fontSize:12, outline:"none" }}>
                            {DROP_OPTIONS.map(d => <option key={d} value={d}>{d}</option>)}
                          </select>
                        </td>
                        <td style={{ ...tdBase }}>
                          <input value={newMed.lu} onChange={e => setNewMed(m => ({ ...m, lu:e.target.value }))} placeholder="e.g. 172"
                            style={{ width:"100%", padding:"5px 8px", borderRadius:6, border:"1.5px solid #f6ad55", fontSize:12, outline:"none", boxSizing:"border-box" }} />
                        </td>
                        <td style={{ ...tdBase }}>
                          <div style={{ display:"flex", gap:5, justifyContent:"center" }}>
                            <button onClick={saveNew} style={{ padding:"4px 10px", borderRadius:5, border:"none", background:"#38a169", color:"#fff", fontSize:11, fontWeight:700, cursor:"pointer" }}>Save</button>
                            <button onClick={() => { setAddingNew(false); setNewMed({ ...BLANK_MED }); }} style={{ padding:"4px 10px", borderRadius:5, border:"none", background:"#e2e8f0", color:"#4a5568", fontSize:11, fontWeight:700, cursor:"pointer" }}>Cancel</button>
                          </div>
                        </td>
                      </tr>
                    )}
                    ...
                    {displayedFormulary.length === 0 && !addingNew && (
                      <tr><td colSpan={6} style={{ padding:"32px", textAlign:"center", color:"#a0aec0", fontSize:14 }}>No medications match your search.</td></tr>
                    )}
                    ...
                    {displayedFormulary.map((med, idx) => {
                      ...
                      return (
                        <tr key={med.id} style={{ background:rowBg, transition:"background 0.15s" }}>
                          {/* Brand */}
                          <td style={{ ...tdBase, background:rowBg, textAlign:"left", paddingLeft:18, fontWeight:700, color:"#1a365d", fontSize:13 }}>
                            {isEditing ? <EditCell field="brand" /> : med.brand}
                          </td>
                          {/* Generic */}
                          <td style={{ ...tdBase, background:rowBg, textAlign:"left", fontSize:12, color:"#4a5568" }}>
                            {isEditing ? <EditCell field="generic" /> : med.generic}
                          </td>
                          {/* Freq */}
                          <td style={{ ...tdBase, background:rowBg }}>
                            {isEditing ? <EditCell field="defaultFreq" type="freq" /> : (
                              <span style={{ padding:"2px 10px", borderRadius:20, background: med.defaultFreq === 1 ? "#c6f6d5" : med.defaultFreq === 2 ? "#bee3f8" : "#fed7aa", fontSize:11, fontWeight:700, color: med.defaultFreq === 1 ? "#276749" : med.defaultFreq === 2 ? "#2b6cb0" : "#c05621", whiteSpace:"nowrap" }}>
                                {med.defaultFreq}x / day
                              </span>
                            )}
                          </td>
                          {/* Drops */}
                          <td style={{ ...tdBase, background:rowBg, fontWeight:600, color:"#2d3748" }}>
                            {isEditing ? <EditCell field="defaultDrops" type="drops" /> : `${med.defaultDrops} drop${med.defaultDrops > 1 ? "s" : ""}`}
                          </td>
                          {/* LU */}
                          <td style={{ ...tdBase, background:rowBg }}>
                            {isEditing ? <EditCell field="lu" /> : (
                              med.lu && med.lu !== "—"
                                ? <span style={{ padding:"2px 10px", borderRadius:20, background:"#e9d8fd", fontSize:11, fontWeight:700, color:"#553c9a" }}>{med.lu}</span>
                          ...
                          <td style={{ ...tdBase, background:rowBg }}>
                            {isEditing ? (
                              <div style={{ display:"flex", gap:5, justifyContent:"center" }}>
                                <button onClick={saveEdit}   style={{ padding:"4px 10px", borderRadius:5, border:"none", background:"#3182ce", color:"#fff", fontSize:11, fontWeight:700, cursor:"pointer" }}>Save</button>
                                <button onClick={cancelEdit} style={{ padding:"4px 10px", borderRadius:5, border:"none", background:"#e2e8f0", color:"#4a5568", fontSize:11, fontWeight:700, cursor:"pointer" }}>Cancel</button>
                              </div>
                            ) : (
                              <div style={{ display:"flex", gap:5, justifyContent:"center" }}>
                                <button onClick={() => startEdit(med)} style={{ padding:"4px 10px", borderRadius:5, border:"1.5px solid #4299e1", background:"#fff", color:"#2b6cb0", fontSize:11, fontWeight:700, cursor:"pointer" }}>Edit</button>
                                <button onClick={() => deleteMed(med.id)} style={{ padding:"4px 10px", borderRadius:5, border:"1.5px solid #fc8181", background:"#fff", color:"#c53030", fontSize:11, fontWeight:700, cursor:"pointer" }}>✕</button>
                              </div>
                            )}
                          </td>
                        </tr>
              ...
              {/* Footer summary */}
              <div style={{ padding:"10px 18px", background:"#f7fafc", borderTop:"1px solid #e2e8f0", display:"flex", gap:20, fontSize:11, color:"#718096", flexWrap:"wrap" }}>
                <span>🟢 1x/day  🔵 2x/day  🟠 3-4x/day</span>
                <span>🟣 LU code required for coverage</span>
                <span style={{ marginLeft:"auto" }}>Changes here auto-populate the Drop Selection tab</span>
              </div>
            </div>
          </div>
        )}
          ...
          <div>
            <div style={{ background:"rgba(255,255,255,0.95)", borderRadius:14, padding:"18px 22px", marginBottom:18, boxShadow:"0 8px 32px rgba(0,0,0,0.25)" }}>
              <div style={{ display:"flex", gap:18, flexWrap:"wrap" }}>
                <FormField label="Patient Name"    value={patientName}    onChange={setPatientName}    placeholder="Enter patient name…"    width="48%" />
                <FormField label="Patient Address" value={patientAddress} onChange={setPatientAddress} placeholder="Enter patient address…" width="48%" />
              </div>
            </div>
              ...
              {[
                ...
                <div key={side} style={{ background:"rgba(255,255,255,0.97)", borderRadius:14, overflow:"hidden", boxShadow:"0 8px 32px rgba(0,0,0,0.25)" }}>
                  ...
                  <div style={{ padding:"16px 18px" }}>
                    <div style={{ display:"grid", gridTemplateColumns:"28px 1fr 100px 82px", gap:8, marginBottom:10, fontSize:11, fontWeight:700, color:"#718096", textTransform:"uppercase", letterSpacing:"0.5px" }}>
                      <div>#</div><div>Medication</div><div>Times / Day</div><div># Drops</div>
                    </div>
                    {drops.map((drop, i) => {
                      const med = formulary.find(m => m.brand === drop.brand);
                      return (
                        <div key={i} style={{ display:"grid", gridTemplateColumns:"28px 1fr 100px 82px", gap:8, marginBottom:9, alignItems:"start" }}>
                          {/* Number badge — offset to vertically centre against the select height (32px) */}
                          <div style={{ width:26, height:26, marginTop:3, background:drop.brand ? color : "#e2e8f0", borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:700, color:drop.brand ? "#fff" : "#a0aec0" }}>{i+1}</div>

                          {/* Medication dropdown + subtitle */}
                          <div>
                            <select value={drop.brand} onChange={e => updateDrop(side, i, "brand", e.target.value)}
                              style={{ width:"100%", padding:"7px 10px", borderRadius:8, border:`1.5px solid ${drop.brand ? color : "#cbd5e0"}`, fontSize:12, color:drop.brand ? "#1a202c" : "#a0aec0", background:drop.brand ? light : "#fff", outline:"none", cursor:"pointer" }}>
                              <option value="">— Select medication —</option>
                              {[...formulary].sort((a,b) => a.brand.localeCompare(b.brand)).map(m =>
                                <option key={m.id} value={m.brand}>{m.brand}</option>
                              )}
                            </select>
                            {/* Generic name + LU badge — always reserve space to keep row height stable */}
                          ...
                          {/* Frequency — aligned to top of select, not subtitle */}
                          <select value={drop.freq} onChange={e => updateDrop(side, i, "freq", Number(e.target.value))} disabled={!drop.brand}
                            style={{ width:"100%", padding:"7px 8px", borderRadius:8, border:`1.5px solid ${drop.brand ? color : "#e2e8f0"}`, fontSize:12, color:drop.brand ? "#1a202c" : "#a0aec0", background:drop.brand ? light : "#f7fafc", outline:"none" }}>
                            {FREQ_OPTIONS.map(f => <option key={f} value={f}>{f}x / day</option>)}
                          </select>

                          {/* Drops — aligned to top of select */}
                  ...
                  {drops.filter(d => d.brand).length > 0 && (
                    <div style={{ margin:"0 18px 16px", padding:"11px 14px", background:light, borderRadius:10, border:`1px solid ${color}30` }}>
                      ...
                      {drops.filter(d => d.brand).map((d, i) => {
                        ...
                        return (
                          <div key={i} style={{ fontSize:12, color:"#2d3748", marginBottom:3, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          ...
          <div>
            <div style={{ background:"rgba(255,255,255,0.95)", borderRadius:14, padding:"16px 22px", marginBottom:18, boxShadow:"0 8px 32px rgba(0,0,0,0.25)", display:"flex", alignItems:"center", gap:14, flexWrap:"wrap" }}>
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                ...
                <select value={scheduleMonth} onChange={e => setScheduleMonth(Number(e.target.value))} style={{ padding:"7px 12px", borderRadius:8, border:"1.5px solid #cbd5e0", fontSize:13, outline:"none" }}>
                  {months.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}
                </select>
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                <label style={{ fontSize:13, fontWeight:600, color:"#4a5568" }}>Year:</label>
                <select value={scheduleYear} onChange={e => setScheduleYear(Number(e.target.value))} style={{ padding:"7px 12px", borderRadius:8, border:"1.5px solid #cbd5e0", fontSize:13, outline:"none" }}>
            ...
            {activeRight.length === 0 && activeLeft.length === 0 ? (
              ...
              <div ref={printRef}>
                <div style={{ background:"rgba(255,255,255,0.97)", borderRadius:14, overflow:"hidden", boxShadow:"0 8px 32px rgba(0,0,0,0.25)" }}>
                  <div style={{ background:"linear-gradient(135deg,#1a365d,#2a4a8a)", padding:"15px 22px", color:"#fff", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                    <div>
                  ...
                  <div style={{ overflowX:"auto" }}>
                    <table style={{ width:"100%", borderCollapse:"collapse", minWidth:600 }}>
                      <thead>
                        <tr>
                          <th style={{ ...thBase, width:110, background:"#2d3748" }}>Date</th>
                          <th style={{ ...thBase, width:50,  background:"#2d3748" }}>Day</th>
                          {activeRight.length > 0 && <th colSpan={activeRight.length} style={{ ...thBase, background:"#1a4a8a", letterSpacing:1 }}>RIGHT EYE (OD)</th>}
                          {activeLeft.length  > 0 && <th colSpan={activeLeft.length}  style={{ ...thBase, background:"#1a5c3a", letterSpacing:1 }}>LEFT EYE (OS)</th>}
                          <th style={{ ...thBase, background:"#2d3748", width:100 }}>Notes</th>
                        </tr>
                        <tr>
                      ...
                      <tbody>
                        {Array.from({ length:numDays }, (_,dayIdx) => {
                          ...
                          return (
                            <tr key={dayIdx}>
                              <td style={{ ...tdBase, background:rowBg, fontWeight:600, color:isSun?"#c53030":isSat?"#2b6cb0":"#2d3748", fontSize:12, whiteSpace:"nowrap" }}>{dateStr}</td>
        ...
        {activeTab === "rx" && (
          <div style={{ display:"grid", gridTemplateColumns:"310px 1fr", gap:22, alignItems:"start" }}>
            ...
            {/* Left: physician form */}
            <div style={{ background:"rgba(255,255,255,0.95)", borderRadius:14, overflow:"hidden", boxShadow:"0 8px 32px rgba(0,0,0,0.25)", position:"sticky", top:20 }}>
              ...
              <div style={{ padding:"16px 18px" }}>
                <FormField label="Physician Name"             value={physicianName}  onChange={setPhysicianName}  placeholder="Dr. Jane Smith, MD" />
                <FormField label="Clinic / Practice Name"     value={clinicName}     onChange={setClinicName}     placeholder="City Eye Care Associates" />
                <FormField label="Street Address"             value={clinicAddress}  onChange={setClinicAddress}  placeholder="123 Medical Drive, Suite 400" />
                <FormField label="City, Province / State"     value={clinicCity}     onChange={setClinicCity}     placeholder="Toronto, ON  M5V 2T6" />
                <FormField label="Phone Number"               value={clinicPhone}    onChange={setClinicPhone}    placeholder="(416) 555-0100" />
                <FormField label="License / DEA / CPSO #"     value={licenseNum}     onChange={setLicenseNum}     placeholder="ON-123456" />
                ...
                <div style={{ marginBottom:12 }}>
                  ...
                  <div style={{ display:"flex", gap:7 }}>
                    {[0,1,2,3,4,5].map(n => (
                      <button key={n} onClick={() => setRxRefills(n)} style={{ width:34, height:34, borderRadius:"50%", border:`2px solid ${rxRefills===n?"#6b46c1":"#cbd5e0"}`, background:rxRefills===n?"#6b46c1":"#fff", color:rxRefills===n?"#fff":"#4a5568", fontWeight:700, fontSize:12, cursor:"pointer", transition:"all 0.15s" }}>
                        {n===0 ? "✕" : n}
                      </button>
                    ))}
                  </div>
                  <div style={{ fontSize:11, color:"#718096", marginTop:5 }}>{rxRefills===0 ? "No refills (Do Not Refill)" : `${rxRefills} refill${rxRefills>1?"s":""} authorized`}</div>
                </div>
                ...
                <div>
                  ...
                  <textarea value={rxNotes} onChange={e => setRxNotes(e.target.value)} placeholder="e.g. Apply gentle pressure to inner corner of eye for 1 minute…" rows={3}
                    style={{ width:"100%", padding:"8px 12px", borderRadius:7, border:"1.5px solid #cbd5e0", fontSize:12, color:"#2d3748", outline:"none", resize:"vertical", fontFamily:"inherit", boxSizing:"border-box" }}
                  />
                </div>
            ...
            <div>
              ...
              <div ref={rxPrintRef}>
                <div style={{ width:500, margin:"0 auto", background:"#fff", border:"2px solid #1a365d", padding:"28px 34px 110px", boxShadow:"0 16px 50px rgba(0,0,0,0.4)", position:"relative", minHeight:700, fontFamily:"'Times New Roman',Georgia,serif", color:"#111" }}>

                  {/* Header */}
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", paddingBottom:12, borderBottom:"2.5px solid #1a365d", marginBottom:14 }}>
                    <div style={{ lineHeight:1 }}>
                      <span style={{ fontSize:60, fontWeight:900, color:"#1a365d", fontStyle:"italic", fontFamily:"'Times New Roman',serif" }}>R</span>
                      <span style={{ fontSize:26, fontWeight:900, color:"#1a365d", fontStyle:"italic", verticalAlign:"super", fontFamily:"'Times New Roman',serif" }}>x</span>
                    </div>
                  ...
                  <div style={{ display:"flex", justifyContent:"flex-end", fontSize:11, marginBottom:14, alignItems:"center", gap:8 }}>
                    <span style={{ fontWeight:700, color:"#333" }}>Date:</span>
                  ...
                  <div style={{ marginBottom:14 }}>
                    <div style={{ marginBottom:9 }}>
                      <span style={{ fontSize:11, fontWeight:700 }}>Patient: </span>
                      <span style={{ display:"inline-block", borderBottom:"1px solid #666", minWidth:320, fontSize:13, paddingBottom:1, paddingLeft:4 }}>{patientName}</span>
                    </div>
                    <div>
                      <span style={{ fontSize:11, fontWeight:700 }}>Address: </span>
                      <span style={{ display:"inline-block", borderBottom:"1px solid #666", minWidth:310, fontSize:12, paddingBottom:1, paddingLeft:4 }}>{patientAddress}</span>
                    </div>
                  </div>
                  ...
                  {mergedDrops.length === 0 ? (
                    <div style={{ fontSize:12, color:"#aaa", fontStyle:"italic", padding:"14px 0" }}>No medications selected — go to Drop Selection tab to add eye drops.</div>
                  ) : mergedDrops.map((d,i) => {
                    const med       = formulary.find(m => m.brand === d.brand);
                    const freqLabel = FREQ_LABEL[d.freq] || `${d.freq}x/day`;
                    return (
                      <div key={i} style={{ marginBottom:15, paddingLeft:14, borderLeft:"3px solid #1a365d" }}>
                        ...
                        <div style={{ fontSize:12, marginTop:5, color:"#111", lineHeight:1.6 }}>
                          Instill <strong>{d.drops}</strong> drop{d.drops>1?"s":""} — <strong>{freqLabel}</strong>
                        </div>
                  ...
                  <div style={{ position:"absolute", bottom:22, left:34, right:34 }}>
                    <div style={{ borderTop:"1.5px solid #1a365d", paddingTop:12, display:"flex", justifyContent:"space-between", alignItems:"flex-end" }}>
                      <div style={{ fontSize:11 }}>
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                          ...
                          {[1,2,3,4,5].map(n => (
                            <span key={n} style={{ width:22, height:22, borderRadius:"50%", border:"1.5px solid #333", display:"inline-flex", alignItems:"center", justifyContent:"center", fontSize:10, fontWeight:700, background:rxRefills>=n?"#1a365d":"#fff", color:rxRefills>=n?"#fff":"#333" }}>{n}</span>
                          ))}
                        </div>
                      </div>
